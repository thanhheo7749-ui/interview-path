...
The system SHALL display an SVG-based AI HR avatar character with a professional, cartoon-style appearance (wearing a suit/vest, neat hair, serious but friendly expression) within a responsive container that adapts to the inline camera widget dimensions.

#### Scenario: Avatar visible in inline camera widget
- **WHEN** the inline camera widget is open
- **THEN** the AI HR avatar SHALL be displayed inside the widget container, scaling its SVG appropriately without overflowing or losing aspect ratio.
...
- **WHEN** the AI responds with TTS audio
...
- **WHEN** the inline camera widget is closed
