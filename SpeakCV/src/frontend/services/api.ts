...
// --- GET HINT ---
export const getHint = async (lastQuestion: string, jdText: string, lang: string = "vi") => {
  const res = await apiRequest(`${API_URL}/hint`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ last_question: lastQuestion, jd_text: jdText, lang }),
  });
  return res.json();
};
...
export const updateSystemConfig = async (
  systemPrompt: string,
  temperature: number,
  aiEnableBrainRetrieval: boolean,
  aiBrainIngestionEnrichmentEnabled: boolean
) => {
  const token = sessionStorage.getItem("token");
  const res = await fetch(`${API_URL}/admin/config`, {
      method: "PUT",
      ...
      body: JSON.stringify({
        ...
        temperature,
        ai_enable_brain_retrieval: aiEnableBrainRetrieval,
        ai_brain_ingestion_enrichment_enabled: aiBrainIngestionEnrichmentEnabled,
      })
  });
...
export const uploadAIBrainFile = async (file: File) => {
  const token = sessionStorage.getItem("token");
  const formData = new FormData();
  formData.append("file", file);
  const res = await fetch(`${API_URL}/admin/ai-brain/ingestion/upload`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}` },
    body: formData,
  });
...
export const createAIBrainIngestionJob = async (fileKey: string, version?: string) => {
  const token = sessionStorage.getItem("token");
  const res = await fetch(`${API_URL}/admin/ai-brain/ingestion/jobs`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
    },
    body: JSON.stringify({ file_key: fileKey, version }),
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: "Create job failed" }));
    throw new Error(err.detail || "Create job failed");
  }
  return res.json();
};

export const getAIBrainIngestionJob = async (jobId: string) => {
  const token = sessionStorage.getItem("token");
  const res = await fetch(`${API_URL}/admin/ai-brain/ingestion/jobs/${jobId}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: "Fetch job failed" }));
    throw new Error(err.detail || "Fetch job failed");
  }
  return res.json();
};

export const applyAIBrainDraft = async (jobId: string, confidenceThreshold: number = 0.65) => {
  const token = sessionStorage.getItem("token");
  const res = await fetch(`${API_URL}/admin/ai-brain/ingestion/jobs/${jobId}/apply-draft`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
    },
    body: JSON.stringify({ accept_all_high_confidence: true, confidence_threshold: confidenceThreshold }),
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: "Apply draft failed" }));
    throw new Error(err.detail || "Apply draft failed");
  }
  return res.json();
};

export const publishAIBrainVersion = async (version: string) => {
  const token = sessionStorage.getItem("token");
  const res = await fetch(`${API_URL}/admin/ai-brain/publish/${encodeURIComponent(version)}`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) {
...
export const getAIBrainVersions = async () => {
  const token = sessionStorage.getItem("token");
  const res = await fetch(`${API_URL}/admin/ai-brain/versions`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: "Load versions failed" }));
    throw new Error(err.detail || "Load versions failed");
  }
  return res.json();
};

export const assembleAIBrainPrompt = async (payload: {
  version?: string;
  job_role?: string;
  seniority?: string;
  ...
  const token = sessionStorage.getItem("token");
  const res = await fetch(`${API_URL}/admin/ai-brain/assemble-prompt`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: "Assemble prompt failed" }));
    throw new Error(err.detail || "Assemble prompt failed");
  }
  return res.json();
};
