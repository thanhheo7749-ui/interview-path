...
def test_build_turn_analysis_keeps_existing_analysis_fields_without_mutating_input():
    ...
    assert set(result["turn_evaluation"]) >= {
        "correctness",
        "depth",
        "communication",
        "topic_relevance",
...
def test_build_turn_analysis_returns_structured_data_only_for_empty_answer():
    ...
    assert {"intent", "confidence_level", "turn_evaluation", "prompt_hint"} <= set(result)
    assert result["turn_evaluation"]["correctness"] == 0.0
    assert result["turn_evaluation"]["depth"] == 0.0
    assert result["turn_evaluation"]["communication"] == 0.0
    assert result["turn_evaluation"]["topic_relevance"] == 0.0
    assert result["prompt_hint"] == PROMPT_HINT
    assert "live_cues" in result
    assert isinstance(result["live_cues"], list)
...
def test_build_turn_analysis_returns_scores_without_prompt_formatting_for_stronger_answer():
    ...
    assert {"intent", "confidence_level", "turn_evaluation"} <= set(result)
    assert result["turn_evaluation"]["depth"] > 0.4
    assert set(result["turn_evaluation"]) >= {
        "correctness",
        "depth",
