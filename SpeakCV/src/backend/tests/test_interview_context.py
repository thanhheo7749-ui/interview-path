...
def test_build_interview_context_returns_priority_topics_and_seed():
    result = analyze_interview_context(
        cv_text="Python, FastAPI, Redis",
        jd_text="Python, Caching, System Design, Redis",
    )

    assert result["highlighted_strengths"] == ["Python", "Redis"]
    assert result["priority_topics"] == ["Caching", "System Design"]
    assert result["question_plan_seed"] == {
        "starter_topic": "Caching",
        "strength_topics": ["Python", "Redis"],
        "gap_topics": ["Caching", "System Design"],
    }
