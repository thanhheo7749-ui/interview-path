...
def update_profile(data: models.ProfileUpdate, current_user: sql_models.User = Depends(get_current_user), db: Session = Depends(database.get_db)):
    ...
    if data.master_cv_text is not None: profile.master_cv_text = data.master_cv_text
    if data.master_cv_structured is not None: profile.master_cv_structured = data.master_cv_structured
