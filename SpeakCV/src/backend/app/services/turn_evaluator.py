...
def _build_live_cues(scores: Dict[str, float], transcript_low_quality: bool = False) -> list[str]:
    cues: list[str] = []
    if scores.get("topic_relevance", 0.0) >= 0.6:
        cues.append("On-topic")
    if scores.get("depth", 0.0) <= 0.4:
...
def _topic_relevance(answer_text: str, target_topic: str) -> float:
    topic_tokens = [token for token in _tokenize(target_topic) if token not in STOP_WORDS]
    answer_token_set = set(_tokenize(answer_text))
    return _clamp(
        sum(1 for token in set(topic_tokens) if token in answer_token_set) / max(len(set(topic_tokens)), 1)
    )
...
def build_turn_analysis(answer_text: str, target_topic: str, analysis: Dict[str, Any]) -> Dict[str, object]:
    turn_evaluation = evaluate_turn(answer_text, target_topic)
    enriched_turn_evaluation = {
        **turn_evaluation,
        "topic_relevance": _topic_relevance(answer_text, target_topic),
    }
    merged_analysis: Dict[str, object] = {
        **analysis,
        "turn_evaluation": enriched_turn_evaluation,
        "live_cues": _build_live_cues(
            enriched_turn_evaluation,
            bool(analysis.get("low_quality_flag", False)),
        ),
    }

    if turn_evaluation["depth"] <= 0.4:
        merged_analysis["prompt_hint"] = PROMPT_HINT
